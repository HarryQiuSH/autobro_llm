...
class RAGEngine:
    ...
    def __init__(
        self, embedding_type: str = "openai", openai_api_key: Optional[str] = None
    ):
        """
    ...
    def _initialize_embeddings(self, openai_api_key: Optional[str]):
    ...
    def create_rag_prompt(self, query: str, context_docs: List[Dict[str, Any]]) -> str:
        ...
        # Create RAG prompt
        rag_prompt = f"""Based on the following context information, please answer the question. If the answer cannot be found in the context, please say so.
...
class RAGChatManager:
    ...
    def process_rag_query(
        ...
        if context_docs:
            augmented_prompt = self.rag_engine.create_rag_prompt(query, context_docs)
        else:
